...
func smCall(t *testing.T, srv *httptest.Server, op, body string) (*http.Response, []byte) {
	...
	require.NoError(t, err, "POST /secretsmanager %s", op)
...
func TestSecretsManager_CreateGetSecret(t *testing.T) {
	...
	require.Equal(t, http.StatusOK, resp.StatusCode, "CreateSecret: %s", body)
	assert.Contains(t, string(body), `"Name":"db-creds"`, "CreateSecret body: %s", body)

	// GetSecretValue.
	resp, body = smCall(t, srv, "GetSecretValue", `{"SecretId":"db-creds"}`)
	require.Equal(t, http.StatusOK, resp.StatusCode, "GetSecretValue: %s", body)
	assert.Contains(t, string(body), `"SecretString":"supersecret"`, "GetSecretValue: %s", body)
}
...
func TestSecretsManager_TerminalStateRefusesRestore(t *testing.T) {
	...
	require.Equal(t, http.StatusOK, resp.StatusCode, "DeleteSecret force")
...
func TestSecretsManager_DestroyedNotFoundContract(t *testing.T) {
	...
	assert.Equal(t, http.StatusNotFound, resp.StatusCode, "DescribeSecret on Destroyed")

	// GetSecretValue on Destroyed → 404.
	resp, _ = smCall(t, srv, "GetSecretValue", `{"SecretId":"d"}`)
	assert.Equal(t, http.StatusNotFound, resp.StatusCode, "GetSecretValue on Destroyed")

	// ListSecrets does NOT include the destroyed secret.
	_, body := smCall(t, srv, "ListSecrets", `{}`)
	assert.NotContains(t, string(body), `"d"`, "ListSecrets must skip Destroyed secrets; body=%s", body)
}
...
func TestSecretsManager_MockStateFiltersDeleted(t *testing.T) {
	...
	_ = resp.Body.Close()
	assert.Contains(t, string(body), `"name":"keep"`, "expected Active secret in /mock/state, got: %s", body)
	...
	body, _ = io.ReadAll(resp.Body)
	_ = resp.Body.Close()
	assert.NotContains(t, string(body), `"name":"pending"`, "expected PendingDeletion secret filtered from /mock/state, got: %s", body)
	...
	body, _ = io.ReadAll(resp.Body)
	_ = resp.Body.Close()
	assert.NotContains(t, string(body), `"name":"destroyed"`, "expected Destroyed secret filtered from /mock/state, got: %s", body)

	// Active "keep" should still be there after the other operations.
	assert.Contains(t, string(body), `"name":"keep"`, "expected Active secret to survive, got: %s", body)
}
...
func TestSecretsManager_VersionStages(t *testing.T) {
	...
	assert.Contains(t, string(body), `"v2"`, "AWSCURRENT: %s", body)
